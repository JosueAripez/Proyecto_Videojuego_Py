...
def mostrar_banderas():
    ...
    for i in range(4):
        ...
        imagen = Image.open(opcion)
        imagen = imagen.resize((150, 100))  # Ajustar el tamaño de la imagen
        imagen_tk = ImageTk.PhotoImage(imagen)
        # Mostrar la imagen en una etiqueta
        label_imagen.configure(image=imagen_tk)
        label_imagen.image = imagen_tk
        # Actualizar las opciones
        botones[i].configure(image=imagen_tk, text=opcion)
        botones[i].image = imagen_tk  # Mantener referencia a la imagen
    return opciones
...
botones = []
for _ in range(4):
    boton = tk.Button(root, text="", width=20, state=tk.DISABLED, compound=tk.TOP)
    boton.pack(pady=5)
